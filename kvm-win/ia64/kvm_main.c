...
MODULE_INFO(version, "kvm-kmod-2.6.32.8");
			      ...
			      struct kvm_assigned_dev_kernel *assigned_dev)
{
	...
	if (assigned_dev->irq_requested_type & KVM_DEV_IRQ_HOST_MSIX) {
		...
		for (i = 0; i < assigned_dev->entries_nr; i++)
			kvm_free_irq(assigned_dev->host_msix_entries[i].vector,
				 (void *)assigned_dev);
		...
		kvm_free_irq(assigned_dev->host_irq, (void *)assigned_dev);
					    ...
					    struct kvm_assigned_dev_kernel *dev)
{
	 ...
	 */
	if (kvm_request_irq(dev->host_irq, kvm_assigned_dev_intr,
					   ...
					   struct kvm_assigned_dev_kernel *dev)
{
	...
	if (!kvm_pcidev_msi_enabled(dev->dev)) {
	...
	dev->host_irq = dev->dev->irq;
	if (kvm_request_irq(dev->host_irq, kvm_assigned_dev_intr, 0,
					    ...
					    struct kvm_assigned_dev_kernel *dev)
{
	...
	for (i = 0; i < dev->entries_nr; i++) {
		r = kvm_request_irq(dev->host_msix_entries[i].vector,
				      ...
				      struct kvm_assigned_pci_dev *assigned_dev)
{
	...
	kvm_INIT_WORK(&match->interrupt_work,
...
IF_ANON_INODES_DOES_REFCOUNTS(	kvm_vcpu_fops.owner = module;)
	...
	printk("loaded kvm module (kvm-kmod-2.6.32.8)\n");
