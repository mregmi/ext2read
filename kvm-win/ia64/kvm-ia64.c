...
int kvm_emulate_halt(struct kvm_vcpu *vcpu)
{
	...
	if (irqchip_in_kernel(vcpu->kvm)) {
		...
		hrtimer_start_p(p_ht, kt, HRTIMER_MODE_ABS);

		vcpu->arch.mp_state = KVM_MP_STATE_HALTED;
		kvm_vcpu_block(vcpu);
		hrtimer_cancel_p(p_ht);
...
static void kvm_migrate_hlt_timer(struct kvm_vcpu *vcpu)
{
	...
	if (hrtimer_cancel_p(p_ht))
		kvm_hrtimer_start_expires(p_ht, HRTIMER_MODE_ABS);
}
...
int kvm_arch_vcpu_init(struct kvm_vcpu *vcpu)
{
	...
	hrtimer_init_p(&vcpu->arch.hlt_timer, CLOCK_MONOTONIC, HRTIMER_MODE_ABS);
...
void kvm_arch_vcpu_uninit(struct kvm_vcpu *vcpu)
{
	...
	hrtimer_cancel_p(&vcpu->arch.hlt_timer);
			 ...
			 unsigned int ioctl, unsigned long arg)
{
	...
	case KVM_IA64_VCPU_GET_STACK: {
		struct kvm_ia64_vcpu_stack   *user_stack;
	        void   *first_p = argp;
