...
static INLINE int 
read_sector(struct disk *dsk, lloff_t sectno, int nsects, void *buf)
{
	return read_disk(dsk->handle, buf, sectno, nsects, dsk->sector_size);
}

static INLINE int 
write_sector(struct disk *dsk, lloff_t sectno, int nsects, void *buf)
{
	return write_disk(dsk->handle, buf, sectno, nsects, dsk->sector_size);
}
